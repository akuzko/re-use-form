...
describe('useForm', () => {
  ...
  describe('validations', () => {
    function Form() {
      const { $, validate } = useForm({ foo: '' }).withValidation({
        foo: 'presence'
      });
    ...
    describe('validation and custom onChange handler', () => {
      function Form() {
        const { $, set, validate } = useForm({ foo: '' }).withValidation({
          foo: 'presence'
        });
    ...
    describe('validating multiple values on setting them at once', () => {
      function Form() {
        const { $, set, validate } = useForm({ foo: '', bar: '' }).withValidation({
          foo: 'presence',
          bar: 'presence'
        });
    ...
    describe('value-dependent validations', () => {
      function Form() {
        const { $, validate } = useForm({ foo: '', bar: '1' }).withValidation(get => {
          if (+get('bar') > 1) {
            return { foo: 'presence' };
          }
          return {};
    ...
    describe('complex validation with wildcards', () => {
      function Form() {
        const { get, set, $, getError, validate } = useForm({ foos: [] }).withValidation({
          'foos': 'presence',
          'foos.*.value': 'presence'
        });
